...
export default defineSuite('ride speed state machine', ({ test }) => {
  ...
  test('speed_apply defers while the ocean sprint owns the gamerule', async ({ mc }) => {
    if (await speedRule(mc) === null) skip('minecart max-speed gamerule missing on this server (see version-compat suite)');
    const before = await speedRule(mc);
    await mc.setScore('.fast', 'ir', 1);
    await mc.fn('speed_inc');
    eq(await mc.score('.speed', 'ir'), 12, '.speed state still changes');
    eq(await speedRule(mc), before, 'gamerule untouched while .fast = 1');
    await mc.setScore('.fast', 'ir', 0);
    await mc.fn('speed_reset');
  });

  test('ocean speed_up applies max(.OCEANSPEED, .speed) and flags .fast', async ({ mc, expected }) => {
    await mc.setScore('.fast', 'ir', 0);
    await mc.fn('speed_up');
    const ocean = expected.get('.OCEANSPEED');
    eq(await mc.score('.ospd', 'ir'), Math.max(ocean, 8), '.ospd = the winning ocean speed');
    eq(await mc.score('.fast', 'ir'), 1, '.fast raised');
    const rule = await speedRule(mc);
    if (rule !== null) eq(rule, Math.max(ocean, 8), 'gamerule got the ocean speed');
  });

  test('ocean speed_up never slows below a faster chosen speed', async ({ mc, expected }) => {
    const ocean = expected.get('.OCEANSPEED');
    await mc.setScore('.speed', 'ir', ocean + 8);
    await mc.fn('speed_up');
    eq(await mc.score('.ospd', 'ir'), ocean + 8, 'chosen land speed wins over a slower ocean speed');
    await mc.fn('speed_reset');
  });
