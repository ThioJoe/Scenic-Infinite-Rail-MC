...
| `44-invistrack` | invisible track (`.HIDETRACK`): new columns carry no rail/support but keep their light, pre-toggle track keeps its rails, the just-in-time strip carries the pace cart across the invisible stretch (placed ahead, wiped behind, zero watchdog rescues), toggle-off restores normal building, and `stop` sweeps the serving strip — with a rail-present control first so the "no rail" probes can't false-pass |
