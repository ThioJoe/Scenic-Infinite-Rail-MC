...
The project version's single source of truth is `header.version` in `src/bedrock/bp/manifest.json` (Java's `pack.mcmeta` carries only a Minecraft-compatibility `pack_format`, never a product version, so it is not part of this). Two CI steps guard it, both emitting `::error::` annotations that name the offending value:
