...
| `dist/ScenicInfiniteRailMode-Java_*.zip` | Or drag this zip onto the Data Packs screen |
...
- The *config knobs* live once, in `src/shared/functions/config.mcfunction` — **the single source of truth for every default**. The Bedrock script keeps a literal mirror of them (`CONFIG_DEFAULTS` in `scripts/main.js`) because its `cfg()` reads need a fallback the scoreboard can't always supply (cmd-bridge worlds, where the Script API cannot see command-written scores; a half-installed pack whose `function config` can't run). That mirror is **not** trusted to stay in sync by hand: `build.mjs` (step 1b) parses both and fails the build on any missing/extra knob, value drift, or a knob whose `cfg_*` objective in config.mcfunction disagrees with the script's `CFG_GROUPS` placement. Retune a knob in config.mcfunction, copy the value into `CONFIG_DEFAULTS`, and the build tells you if you forgot.
