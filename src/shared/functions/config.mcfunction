...
# The auto-started ride begins at X = -99,000: the countdown pre-generates
# a starting area there and teleports the first player in at its end,
# straight into the launch seat. Heading east forever, that roughly
# doubles the time spent at low absolute coordinates, where Bedrock's
# 32-bit floating-point positions are at their most precise (Java mirrors
# it for parity). Manual starts are never relocated -- they anchor where
# you stand.
