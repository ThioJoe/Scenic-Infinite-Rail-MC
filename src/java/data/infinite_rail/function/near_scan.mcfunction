...
# CONTEXT.md section 7j). Reads the surface every 2 blocks over the next
# .SAMPLE_WINDOW blocks east of the head (odd offsets +1, +3, +5, ...) from
# the ROLLING SURFACE CACHE (surf_roll -- the same heightmap + not-terrain
# dig-down every sample has always used, probed once per X instead of once
# per column, so trees and structures are already invisible). Consecutive
# reads are folded into PAIRS -- min(this, prev) -- to erase what the
# dig-down can't: a 1-2 block wide spike of REAL terrain (rock fins, lone
# pillars) only ever catches one probe of a pair, so the min drops it, while
# real ground (4+ blocks wide) spans both probes and registers. The pairs
...
# positioned at the head marker, like sample_window (the lazy fill's probe
# offset is relative to it). The reach IS the sample window (floored at 1),
# so the scan never reads past the sampling horizon -- keep
# .SAMPLE_WINDOW <= .TERRAIN_GENAHEAD so both stay inside the generated
# corridor.
...
# A fresh scratch copy of the cache for this walk to consume (sample_window
# ran surf_roll for this column, and its lazy fills are already in c).
data modify storage infinite_rail:surf w set from storage infinite_rail:surf c
execute if score .nw ir matches 1.. run function infinite_rail:near_step
