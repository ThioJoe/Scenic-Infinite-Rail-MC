...
execute if score .DEBUGMODE ir matches 1 if score .fast ir matches 0 run tellraw @a [{"text":"[SR Debug] ","color":"dark_aqua"},{"text":"switching to fast ocean mode, speed ","color":"aqua"},{"score":{"name":".OCEANSPEED","objective":"cfg_ride"},"color":"white"}]
