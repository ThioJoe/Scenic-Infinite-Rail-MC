...
#      (within .DEADBAND below the hover line) for .GAPSTRETCH columns --
...
# inside the generated corridor: .GENAHEAD, default 192, covers it).
...
# Gates: feature on; running flat; a descent of at least .DEADBAND wanted.
...
execute if score .sD ir < .DEADBAND cfg_terrain run return 0
...
scoreboard players operation .sband ir -= .DEADBAND cfg_terrain
