...
#      (beyond .PLOW_GRACE_DOWN levels of cut-through), so the floor guard
#      cannot cut it into pieces -- the shifted descent is the SAME single
#      event to the SAME landing;
#   2. the landing really is a BOTTOM: ground sits at the landing level
#      (within .MIN_CHANGE below the hover line) for .SHIFT_REQ_BOTTOM
#      columns -- this many columns of straight ridable track at the
#      bottom, so the calm the gap exists to guarantee simply happens
