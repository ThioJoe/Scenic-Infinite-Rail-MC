...
The books' pages are 1.21.5+-format SNBT text components: clickable links (each a `click_event:{action:"run_command",command:"trigger ir_menu set <n>"}` — no leading slash; the page root is an empty `{text:""}` so its style can't inherit into the children) plus short descriptive `hover_event` tips (the concrete `/scoreboard` examples live behind the Debug book's `[Command help]` instead). The Settings page has `[On]`/`[Off]` per mode, a `Density: [Low] [Med] [High] [Max]` row under Torches (the `torch_density_*` presets — friendly names only, no percentages), the tri-state `Time: [Night] [Day] [Default]` row, a `Speed: [Reset]` row (adjusting is the hotbar items' job) and `[Current modes]`. The Speed items are **re-modeled `carrot_on_a_stick`s** — the one item whose `used:` statistic increments on any right-click — disguised as a dye / an emerald via the `minecraft:item_model` component and told apart by `custom_data` (§6.10).
...
- **The pinned hotbar items open native forms / nudge the speed.** The inventory keeper pins four named items instead of clearing their slots — the "Settings" and "Debug" books plus the "Speed −" (red dye) and "Speed +" (emerald) items, mirroring Java's hotbar layout (deliberately *not* slot-locked — `ItemLockMode` items get a lock badge and a "Can't be moved/dropped/…" tooltip block, and the per-tick keeper makes the lock redundant anyway). Using one fires `world.afterEvents.itemUse`, matched by item type + name + rider: the Settings book shows a `@minecraft/server-ui` **ModalFormData** (toggles per mode, the tri-state Time dropdown, the Low/Medium/High/Max torch-density dropdown — friendly names only, running the same `torch_density_*` files as Java's book, and a 1–64 speed slider whose label names the config default) pre-set from the live scores, with only actual changes running their function files (so the tellraw feedback matches the chat commands exactly — the slider feeds the shared `speed_step` as a delta); the Debug book shows the debug form (chat-output toggle, the sidebar-view dropdown, a command-help printer); the Speed items run `speed_inc`/`speed_dec` (with a short cooldown, since holding "use" refires `itemUse`). The BP manifest therefore depends on `@minecraft/server-ui` `2.0.0` (stable long before the pack's 1.21.120 floor). `stop` removes all four.
