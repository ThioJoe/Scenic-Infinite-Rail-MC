...
| `.speed`     | The **adjustable ride speed** (blocks/s, clamped 1..64 by the shared `speed_step`): what `begin` applies at start and `speed_down`/`mode_sky_off` restore. Seeded from `.MAXSPEED` by `modes_init` the first time; nudged by the Speed − / Speed + hotbar items and the Settings menu, reset to the default from there too. State like the modes — survives `/reload`, ride restarts and rejoins. |
...
Pins the rider's four hotbar items (`item replace …`), re-given every tick right after `main`'s inventory clear, so at every tick boundary they exist and nothing else ever accumulates (`stop` takes them all back):

| Slot | Item | Job |
| ---- | ---- | --- |
| `hotbar.0` | **"Speed −"** | one block/s slower per right-click (§6.10) |
| `hotbar.1` | **"Speed +"** | one block/s faster per right-click (§6.10) |
...
The books' pages are 1.21.5+-format SNBT text components: clickable links (each a `click_event:{action:"run_command",command:"trigger ir_menu set <n>"}` — no leading slash; the page root is an empty `{text:""}` so its style can't inherit into the children) plus `hover_event` tips — the Debug book's sidebar links, for instance, show the exact `/scoreboard players set …` command for their group on hover. The Settings page has `[On]`/`[Off]` per mode, the tri-state `Time: [Night] [Day] [Default]` row, a `Speed: [-] [+] [Reset]` row and `[Current modes]`. The Speed items are **re-modeled `carrot_on_a_stick`s** — the one item whose `used:` statistic increments on any right-click — disguised as a dye / an emerald via the `minecraft:item_model` component and told apart by `custom_data` (§6.10).
...
### 7f. The keepers
Per-tick guards in `main` make the ride truly unbreakable: anything riding the pace cart that isn't the plug is ejected, as is anything riding the ride cart that isn't a player; a dismounted rider is re-mounted into the ride cart; the plug and the ride cart are re-mounted onto their perches (unconditional attempts that fail silently while already seated); and if the pace cart's eastward speed ever drops near zero it's re-boosted to `0.5`. The ride cart's pitch is locked horizontally, and the player's inventory is cleared every tick — then the **pinned hotbar items** (the Settings/Debug books and the Speed −/+ items, §6.9/§6.10) are handed straight back by `give_menu`, so they are always there and nothing else ever accumulates. Combined with the always-powered rails, the ride can never stop — and because both carts always carry a passenger, neither can be entered by right-click or scoop up passing mobs.
