...
Seeds all four toggle scores with `scoreboard players add … 0` — creates a missing score at 0, leaves a set one alone — and seeds the adjustable ride speed the same way (`.speed` copies `.MAXSPEED` only when unset or invalid, so a chosen speed persists — §6.10). Called from `load` (Java) and the script's `init()` (Bedrock), always after `config`.
...
The books' pages are 1.21.5+-format SNBT text components: clickable links (each a `click_event:{action:"run_command",command:"trigger ir_menu set <n>"}` — no leading slash; the page root is an empty `{text:""}` so its style can't inherit into the children) plus short descriptive `hover_event` tips (the concrete `/scoreboard` examples live behind the Debug book's `[Command help]` instead). The Settings page has `[On]`/`[Off]` per mode, the tri-state `Time: [Night] [Day] [Default]` row, a `Speed: [Reset]` row (adjusting is the hotbar items' job) and `[Current modes]`. The Speed items are **re-modeled `carrot_on_a_stick`s** — the one item whose `used:` statistic increments on any right-click — disguised as a dye / an emerald via the `minecraft:item_model` component and told apart by `custom_data` (§6.10).
...
| The pinned hotbar items: Settings/Debug menus + Speed −/+ | **two written books** (clickable `click_event`s that `/trigger` the `ir_menu` objective, dispatched by `menu_tick` — permission-free: no operator, no 1.21.6+ confirmation screen) plus **two re-modeled `carrot_on_a_stick`s** (the `used:` stat objective `ir_click` → `speed_click`, told apart by `custom_data`), all pinned by `give_menu` after each per-tick inventory clear | **four plain named items** pinned by the inventory keeper; using one fires `itemUse` — the books open native `@minecraft/server-ui` **ModalForms** pre-set from the live scores (Settings: mode toggles + the tri-state Time dropdown + a speed slider whose label shows the default; Debug: chat toggle + the sidebar-view dropdown), applying only actual changes by running the same function files; the speed items run `speed_inc`/`speed_dec` directly |
