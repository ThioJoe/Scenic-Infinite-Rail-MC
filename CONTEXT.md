...
- **`cfg_ride`** (11 rows) — speed, mode knobs, performance: `.MAXSPEED` `.OCEANSPEED` `.OCEANCHUNKS` `.LANDCHUNKS` `.SKYY` `.SKYSPEED` `.TORCHODDS` `.TORCHRANGE` `.AHEAD` `.GENAHEAD` `.MAXTICK`
...
Optional flavors toggled by chat command (`/function infinite_rail:mode_*`) — or through the **Settings book**, the in-game menu item pinned into the rider's hotbar (`give_menu`, §6.10). Rain, torches, sky and hide-cart are `_on`/`_off` pairs; time is a **tri-state** (night only / day only / default — the three-way `.NIGHTMODE`). They are **independent switches, not a mutually exclusive mode select** — any combination stacks — and they are **state, not config**: the toggles live in the `.RAINMODE`/`.NIGHTMODE`/`.TORCHMODE`/ `.SKYMODE`/`.HIDECART` scores (§4.1), seeded by the shared `modes_init` and untouched by `config`, so a `/reload`, a ride restart, `stop`, or a rejoin never turns a mode off. The knobs shaping them (`.SKYY`, `.SKYSPEED`, `.TORCHRANGE`) are ordinary config tunables; the torch **density** is menu-driven state (`.torchdens`, the Low/Medium/High/Max presets below) with `.TORCHODDS` as its config-side default.

**`modes_init.mcfunction`** *(shared source: `src/shared/functions/`)*
Seeds all five toggle scores with `scoreboard players add … 0` — creates a missing score at 0, leaves a set one alone — and seeds the two adjustable state values the same way: `.speed` copies `.MAXSPEED` and `.torchdens` copies `.TORCHODDS`, each only when unset or invalid, so a chosen speed/density persists (§6.10/§6.9). Called from `load` (Java) and the script's `init()` (Bedrock), always after `config`.
...
Status printout: one `tellraw` line with the rain/torches/sky toggles, plus a second line spelling out the tri-state time mode in words and the adjustable ride speed (with "(default)" when `.speed` equals `.MAXSPEED`).
...
The books' pages are 1.21.5+-format SNBT text components: clickable links (each a `click_event:{action:"run_command",command:"trigger ir_menu set <n>"}` — no leading slash; the page root is an empty `{text:""}` so its style can't inherit into the children) plus short descriptive `hover_event` tips (the concrete `/scoreboard` examples live behind the Debug book's `[Command help]` instead). The Settings page has `[On]`/`[Off]` per mode, a `Density: [Low] [Med] [High] [Max]` row under Torches (the `torch_density_*` presets — friendly names only, no percentages), the tri-state `Time: [Night] [Day] [Default]` row, a `Cart: [Hide] [Show]` row (`.HIDECART`), a `Speed: [Reset]` row (adjusting is the hotbar items' job) and `[Current modes]`. The Speed items are **re-modeled `carrot_on_a_stick`s** — the one item whose `used:` statistic increments on any right-click — disguised as a dye / an emerald via the `minecraft:item_model` component and told apart by `custom_data` (§6.10).
...
| The pinned hotbar items: Settings/Debug menus + Speed −/+ | **two written books** (clickable `click_event`s that `/trigger` the `ir_menu` objective, dispatched by `menu_tick` — permission-free: no operator, no 1.21.6+ confirmation screen) plus **two re-modeled `carrot_on_a_stick`s** (the `used:` stat objective `ir_click` → `speed_click`, told apart by `custom_data`), all pinned by `give_menu` after each per-tick inventory clear | **four plain named items** pinned by the inventory keeper; using one fires `itemUse` — the books open native `@minecraft/server-ui` **ModalForms** pre-set from the live scores (Settings: mode toggles + the tri-state Time dropdown + the torch-density dropdown + a speed slider whose label shows the default; Debug: chat toggle + the sidebar-view dropdown), applying only actual changes by running the same function files; the speed items run `speed_inc`/`speed_dec` directly |
